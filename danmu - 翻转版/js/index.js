    ...
    function showMarks(){
        ...
        function show(){
            ...
            for(var i = start; i < end; i++){
                createMark(i,marks[i],deley);
                deley += 200;
            }
        }
    }
    function createMark(index,markData,delay){
        var newMark = document.createElement("mark");
        var colors = ['markColor1','markColor2','markColor3'];
        newMark.innerHTML = markData.text;
        newMark.style.left = markData.position.x + "px";
        newMark.style.top = markData.position.y + "px";
        newMark.className = colors[index%colors.length];
        picLists[now].appendChild(newMark);
        var scale = (Math.round(Math.random()*100) + 50)/100;
        var x = Math.ceil(Math.random()*60) - 30;
        var y = Math.ceil(Math.random()*40) + 20;
        var degX = Math.ceil(Math.random()*10) - 5;
        var degY = Math.ceil(Math.random()*20) - 10;
        var degZ = Math.ceil(Math.random()*40) - 20;
        newMark.style.opacity = 0;
        newMark.style.webkitTransform = newMark.style.transform = " rotateX("+degX+"deg) rotateY("+degY+"deg) rotateZ("+degZ+"deg) translate("+x+"px,"+y+"px) scale("+scale+")";
        newMark.timer = setTimeout(function(){
            newMark.classList.add("show");
            newMark.timer = setTimeout(function(){
                var x = Math.ceil(Math.random()*30)-15;
                var y = Math.ceil(Math.random()*40) + 20;
                var scale = (Math.round(Math.random()*50) + 100)/100;
                var degX = Math.ceil(Math.random()*10) - 5;
                var degY = Math.ceil(Math.random()*20) - 10;
                var degZ = Math.ceil(Math.random()*40) - 20;
